...
public:
    ...
    void getSeedFileNames(std::vector<std::string> &seeds);
...
void GameOfLife::getSeedFileNames(std::vector<std::string> &seeds){
...
int main(int argc, char* argv[]){	//waitMS and size
	
	int waitMS = atoi(argv[1]);	//microsecs (1,000,000 = 1 sec)
	
	int h = 50,  w = 130, cycles =  5000;	// waitMS = 500000;	
	int row, col;
...
void printMenu(int mrow, int mcol){
	...
	char menuExit[]="[e]xit ";	//xtra space moves cursor out one spot
...
void printPresetMenu(int mrow, int mcol){
	char mGlider[]="[g]lider";
	//char menuPreset[]="[p]ick preset";
	//char menuSettings[]="[s]ettings";
	char menuExit[]="[b]ack";
	
	mvprintw(mrow,mcol,"%s",mGlider);
	//mvprintw(mrow+2,mcol,"%s",menuPreset);
	//mvprintw(mrow+4,mcol,"%s",menuSettings);
	mvprintw(mrow+6,mcol,"%s",menuExit);
