...
void GameOfLife::setInitParamsRandom(){
	...
	for (int c = 0 ; c < columns ; c++ ){
		for (int r = 0 ; r < rows ; r++){
			if((rand() % 3 + 1) == 0){ v.at(c).at(r) = live; }
		}	//end for loop r
	}	//end for loop c
}
...
int main(int argc, char* argv[]){
	...
	bool small = false;
	char ch;
	int h, w, cycles, row, column, waitMS;	//waitMS is in ms (1,000,000 = 1 sec)
	...
	int mrow = 3 + h/2 - 3, mcol = (col - 16)/2;	//row and column for menu
...
void printMenu(int mrow, int mcol){	//width should be 16
	...
	mvprintw(mrow+7,mcol,"  %s",menuExit);
