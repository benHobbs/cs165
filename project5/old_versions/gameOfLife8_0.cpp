...
void printPresetMenu(int mrow, int mcol, int h, int w);
	...
	if(row > 59 && col > 149 || small == true){	//check screen size
		do{
			...
			case 'r':{	//generate random seed
				...
				break;}
			case 'u':{	//use presets
				printPresetMenu(mrow, mcol,h,w);
				break;}
			case 's':{	//check settings
				break;}
			case 'e':{
				break;}
			default:
 ...
 ** Description:    Builds the menu
 *************************************************************************************************/
void printPresetMenu(int mrow, int mcol, int h, int w, int row, int col){
		GameOfLife game(h,w);			//build game of life object
		game.setLiveDeadChars('X',' ');		//set symbols
		game.initVectors();			//initialize vectors
		...
		for(i=0;i < files.size();i++) {
			...
			mvprintw(mrow+i,mcol,  " [%d] %s",i,str.c_str());
		}
		mvprintw(h+6+i+2,mcol," [%d] back         ");
